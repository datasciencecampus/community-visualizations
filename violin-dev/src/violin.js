...
const drawViz = data => {
    ...
    if (data.tables.DEFAULT.length != rowData.length){
        var rowData = data.tables.DEFAULT;
        console.log(rowData)
...
//    console.log(rowData)
    ...
    var scatterNumber =  data.style.scatterNumber.value
    ? data.style.scatterNumber.value
    : data.style.scatterNumber.defaultValue;
    ...
    var axisTextColor =  data.style.axisTextColor.value
    ? data.style.axisTextColor.value.color
    : data.style.axisTextColor.defaultValue;

    var axisTextSize =  data.style.axisTextSize.value
    ? data.style.axisTextSize.value
    : data.style.axisTextSize.defaultValue;

    var axisTextFamily =  data.style.axisTextFamily.value
    ? data.style.axisTextFamily.value
    : data.style.axisTextFamily.defaultValue;

    var axisIsDate =  data.style.axisIsDate.value
    ? data.style.axisIsDate.value
    : data.style.axisIsDate.defaultValue;

    // get the width and the height of the iframe
    var margin = {top: 10, right: 40, bottom: 80, left: 60};
    ...
    // 'violinDimension' comes from the id defined in violin.json
    ...
    var date_axis = function() {
      var self = d3.select(this),
        text = self.text();
      if (axisIsDate == true) {
        var a = moment(text, "YYYYMMDD").format("D MMM YYYY");
        self.text(a);
      } else {
        self.text(text)
      }
    ...
    var sumstat = d3.nest()  // nest function allows to group the calculation per level of a factor
        ...
        input = d.map(function(g) { return g.values;})    // Keep the variable called Sepal_Length
    ...
    var tool_tip = d3.tip()
      .attr("class", "d3-tip")
      .html(d => 'median: '+ d.value.toFixed(2));
      svg.call(tool_tip);
    ...
    var selected = shuffled.slice(0, scatterNumber);
